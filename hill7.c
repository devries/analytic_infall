...
void hill7_init(int channels, double *varray, double *tarray, double nu, double vmin, double vmax) {
  ...
  if(hill_array==NULL) {
    fprintf(stderr, "hill7_init: Out of memory.\n");
    exit(1);
  }
...
double hill7_model(double tau_core, double tau_env, double v_lsr, double v_in, double sigma, double to, double tpeak) {
  double taufr;
  double tauF;
  double tauR;
  int i;
  ...
  for(i=0;i<nchan;i++) {
    ...
    hill_array[i]=(jfunc(tpeak,frequency)-jfunc(to,frequency))*sub*(1.0-exp(-taufr))*exp(-tauF)+(jfunc(to,frequency)-jfunc(TBG,frequency))*(1.0-exp(-2.0*taufr-tauF-tauR));
...
double hill7_evaluate(double *params) {
  ...
  resrms = hill7_model(tau_core,tau_env,v_lsr,v_in,sigma,to,tpeak);
