...
int main(int argc, char *argv[]) {
    ...
    if(argc!=11) {
        fprintf(stderr,"Usage: %s <infile> <frequency> <thin_tex> <vmin> <vmax> <popingen> <genpercheck> <checkperconv> <minpeak> <outbase>\n",argv[0]);
        exit(1);
    }
    ...
    sprintf(fitsoutname,"!%s-fits.fits",argv[10]);
    sprintf(tauoutname,"!%s-tau.fits",argv[10]);
    sprintf(vlsroutname,"!%s-vlsr.fits",argv[10]);
    sprintf(vinoutname,"!%s-vin.fits",argv[10]);
    sprintf(sigmaoutname,"!%s-sigma.fits",argv[10]);
    sprintf(tpeakoutname,"!%s-tpeak.fits",argv[10]);
    sprintf(chisqoutname,"!%s-chisq.fits",argv[10]);
    ...
    sprintf(historyline,"  Minimum peak channel: %s",argv[9]);
    checkfits(fits_write_history(fitsout,historyline,&status));
    checkfits(fits_write_history(tauout,historyline,&status));
    checkfits(fits_write_history(vlsrout,historyline,&status));
    checkfits(fits_write_history(vinout,historyline,&status));
    ...
    /* We always pull the full spectrum */
    fpixel[2]=1L;
    lpixel[2]=naxes[2];
    /* Read in initial fit spectrum */
    /* We need to do this for every spectrum now... move to loop */
    /*
