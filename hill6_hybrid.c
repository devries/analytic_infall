...
double hill6_gsl(const gsl_vector *x, void *junk) {
  return hill6_evaluate(x->data);
}
...
int main(int argc, char *argv[]) {
  ...
  hill6_init(nchan,input_data[0],input_data[1],atof(argv[2]),vmin,vmax);

  devo2_init(&dstruct,6,min,max,popingen,0.2,0.8,hill6_evaluate);
